...
def init_pca9685(bus, freq_hz=1000.0):
    """
...
def load_drivecycle_mat_files(base_folder):
    """
    ...
    if not drivecycle_dir.is_dir():
        raise FileNotFoundError("Cannot find directory: {}".format(drivecycle_dir))
    ...
    for mat_file in drivecycle_dir.glob("*.mat"):
        try:
            ...
            print("Warning: '{}' might be MATLAB v7.3. Skipping.".format(mat_file.name))
            continue

        key = mat_file.stem
        mat_data[key] = data_dict
        user_vars = [k for k in data_dict.keys() if not k.startswith("__")]
        print("[Main] Loaded '{}' → variables = {}".format(mat_file.name, user_vars))
...
def get_gains_for_speed(ref_speed):
    """
...
def hankel_full(ud, yd, Tini, THorizon):
    ...
    print("Hankel full matrix with shape: Up{}, Uf{},Yp{},Yf{}".format(Up.shape, Uf.shape, Yp.shape, Yf.shape))
    return Up, Uf, Yp, Yf
...
def get_gains_for_speed_slower_frequency(ref_speed, Ts):
    """
