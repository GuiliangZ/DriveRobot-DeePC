...
if __name__ == "__main__":
    ...
    hankel_subB_size = 20                             # >(Tini+THorizon)*2-hankel sub-Block column size at each run-time step (199-299)!!! very important hyperparameter to tune. When 
    ...
    controller = AdditiveEnhancementController(Ts=Ts,T_f=T_f,FeedFwdTime=FeedFwdTime,enable_enhancements=True)
    ...
    # ─── MAIN 100 Hz CONTROL LOOP ─────────────────────────────────────────────────
        ...
        try:
            while True:
                ...
                Up_cur, Uf_cur, Yp_cur, Yf_cur = hankel_subBlocks(Up, Uf, Yp, Yf, Tini, THorizon, hankel_subB_size)
                # For Debug Purpose
                # print(
